export type Language = "ru" | "en";
export type Theme = "light" | "dark";

export function normalizeLanguage(value: string | undefined): Language {
  return value === "en" ? "en" : "ru";
}

export function normalizeTheme(value: string | undefined): Theme {
  return value === "dark" ? "dark" : "light";
}

export const dictionary = {
  ru: {
    brand: "Билеты",
    navEvents: "События",
    navAdmin: "Админ",
    navScan: "Сканер QR",
    language: "Язык",
    theme: "Тема",
    darkMode: "Тёмная",
    lightMode: "Светлая",
    switchToDark: "Тёмная тема",
    switchToLight: "Светлая тема",
    upcomingEvents: "Ближайшие события",
    noDescription: "Без описания.",
    bookSeats: "Купить билет",
    venueLabel: "Площадка",
    yourDetails: "Ваши данные",
    fullName: "Полное имя",
    whatsappNumber: "Номер WhatsApp",
    createBooking: "Создание брони...",
    confirmBooking: "Подтвердить бронь",
    clearSelection: "Очистить",
    bookingFormIncomplete: "Выберите места и заполните имя и номер WhatsApp.",
    selectSeatsRequired: "Выберите хотя бы одно место.",
    nameRequired: "Введите имя.",
    phoneRequired: "Введите корректный номер WhatsApp.",
    selectedSeats: "Выбрано мест",
    price: "Стоимость",
    total: "Итого",
    bookingSuccess: "Бронь успешно создана, ожидайте подтверждения",
    name: "Имя",
    whatsapp: "WhatsApp",
    event: "Событие",
    venue: "Площадка",
    place: "Место проведения",
    addressLine: "Адрес",
    status: "Статус",
    commentary: "Комментарий",
    cause: "Причина",
    unknown: "Неизвестно",
    manualPaymentTitle: "Инструкция по оплате",
    manualPaymentText:
      "После подтверждения брони, администратор напишет вам в WhatsApp для оплаты.",
    manualPaymentTextSuccess:
      "Бронь создана и будет ожидать подтверждения администратором, который отправит запрос на оплату.",
    openTicketPage: "Открыть страницу билета",
    ticketNotActive: "Билет пока не активен",
    ticketNotActiveText:
      "Текущий статус брони: {status}. QR-код появится после подтверждения оплаты администратором.",
    yourTicket: "Ваш билет",
    date: "Дата",
    qrMissing: "Для этого билета не найден QR-код.",
    ticketEntryHint: "Покажите этот QR-код на входе для проверки билета.",
    ticketScanStatus: "Статус прохода",
    ticketScanned: "Билет уже использован",
    ticketNotScanned: "Билет еще не использован",
    yourSeats: "Ваши места",
    noSeatDetails: "Детали мест недоступны",
    adminDashboard: "Панель администратора",
    adminLoginTitle: "Вход администратора",
    adminLoginHint: "Только администраторы могут получить доступ к панели.",
    adminUsernameLabel: "Логин администратора",
    adminPasswordLabel: "Пароль администратора",
    adminLoginButton: "Войти",
    adminLoginError: "Неверный логин или пароль. Попробуйте еще раз.",
    adminLogoutButton: "Выйти из админки",
    manageEvents: "Управление событиями",
    manageVenues: "Управление площадками",
    manageBookings: "Управление бронями",
    scanTicketQr: "Сканирование QR билета",
    scanHint:
      "Используйте камеру устройства, чтобы проверить и погасить QR-код билета.",
    qrValidUsed: "Билет валиден и отмечен как использованный.",
    invalidTicket: "Недействительный билет.",
    scanFailed: "Ошибка сканирования",
    scanConfirmQuestion: "Отметить билет как использованный?",
    scanConfirmYes: "Да, погасить билет",
    scanConfirmNo: "Нет",
    scanTicketInfo: "Информация о билете",
    section: "Сектор",
    seats: "Места",
    cameraStartFailed: "Не удалось запустить камеру",
    cameraPermissionDenied:
      "Доступ к камере запрещен. Разрешите доступ в настройках браузера для этого сайта и перезагрузите страницу.",
    createEvent: "Создать событие",
    editEvent: "Редактировать событие",
    edit: "Редактировать",
    saveEventChanges: "Сохранить изменения",
    cancelEditing: "Отменить редактирование",
    editingEventBadge: "Режим редактирования",
    eventName: "Название события",
    description: "Описание",
    eventImage: "Изображение события",
    removeImage: "Удалить изображение",
    selectVenue: "Выберите площадку",
    activeEvent: "Активное событие",
    saving: "Сохранение...",
    createEventBtn: "Создать событие",
    existingEvents: "Существующие события",
    active: "активно",
    inactive: "неактивно",
    markActive: "Сделать активным",
    markInactive: "Сделать неактивным",
    venueEditor: "Редактор площадки",
    createNewVenue: "Создать новую площадку",
    new: "Новая",
    venueName: "Название площадки",
    venueAddressLine: "Адрес площадки",
    layoutMeta: "JSON метаданных схемы",
    seatsJson: "JSON массива мест",
    generateGrid: "Сгенерировать сетку на 48 мест",
    updateVenue: "Обновить площадку",
    createVenueBtn: "Создать площадку",
    seatPreview: "Предпросмотр схемы мест",
    filterStatus: "Фильтр статуса",
    all: "Все",
    pending: "ожидает оплату",
    paid: "оплачено",
    cancelled: "отменено",
    seatsCount: "мест",
    row: "Ряд",
    seatPlace: "место",
    markPaid: "Отметить как оплачено",
    cancel: "Отменить",
    share: "Поделиться",
    print: "Печать",
    linkCopied: "Ссылка скопирована",
    cancelBooking: "Отменить бронь",
    cancelCause: "Укажите причину отмены",
    cancelCausePlaceholder: "Опишите причину...",
    cancelCauseOther: "Другое",
    venueNameRequired: "Название площадки обязательно.",
    vip: "VIP",
    regular: "Обычные",
    balcony: "Балкон",
    seatType: "Тип места",
    addTypePrice: "Добавить тип и стоимость",
    remove: "Удалить",
    typePriceRequired: "Добавьте хотя бы одну цену (по типу мест или по ряду).",
    rowPrices: "Цены по рядам",
    typeColors: "Цвета типов мест",
    bookedSeat: "Занято",
    paidSeat: "Оплачено",
    availableSeat: "Свободно",
    selectedSeat: "Выбрано",
    noSelectedSeats: "Пока ничего не выбрано",
    availableSeats: "Свободные места",
    takenPlaces: "Занятые места",
    noTakenPlaces: "Нет занятых мест",
    currentTimezone: "Текущий часовой пояс: {timezone}",
  },
  en: {
    brand: "Tickets",
    navEvents: "Events",
    navAdmin: "Admin",
    navScan: "Scan QR",
    language: "Language",
    theme: "Theme",
    darkMode: "Dark",
    lightMode: "Light",
    switchToDark: "Dark theme",
    switchToLight: "Light theme",
    upcomingEvents: "Upcoming Events",
    noDescription: "No description.",
    bookSeats: "Book Seats",
    venueLabel: "Venue",
    yourDetails: "Your details",
    fullName: "Full name",
    whatsappNumber: "WhatsApp number",
    createBooking: "Creating booking...",
    confirmBooking: "Confirm booking",
    clearSelection: "Clear",
    bookingFormIncomplete:
      "Please select seats and fill in your name and WhatsApp number.",
    selectSeatsRequired: "Please select at least one seat.",
    nameRequired: "Please enter your name.",
    phoneRequired: "Please enter a valid WhatsApp number.",
    selectedSeats: "Selected seats",
    total: "Total",
    bookingSuccess: "Reservation successfully created, awaiting confirmation",
    name: "Name",
    whatsapp: "WhatsApp",
    event: "Event",
    venue: "Venue",
    place: "Place",
    addressLine: "Address",
    status: "Status",
    commentary: "Commentary",
    cause: "Cause",
    unknown: "Unknown",
    manualPaymentTitle: "Payment instructions",
    manualPaymentText:
      "After confirmation of your booking, our admin will message you on WhatsApp for payment.",
    manualPaymentTextSuccess:
      "The reservation has been created and is awaiting administrator confirmation, who will send a payment request.",
    openTicketPage: "Open ticket page",
    ticketNotActive: "Ticket not active yet",
    ticketNotActiveText:
      "Your booking status is {status}. The QR code appears after manual payment confirmation by admin.",
    yourTicket: "Your Event Ticket",
    date: "Date",
    qrMissing: "QR data is missing for this ticket.",
    ticketEntryHint: "Show this QR code at the entrance for ticket validation.",
    ticketScanStatus: "Entry status",
    ticketScanned: "Ticket already used",
    ticketNotScanned: "Ticket not used yet",
    yourSeats: "Your seats",
    noSeatDetails: "Seat details are unavailable",
    adminDashboard: "Admin Dashboard",
    adminLoginTitle: "Admin login",
    adminLoginHint: "Only admins can access the dashboard.",
    adminUsernameLabel: "Admin username",
    adminPasswordLabel: "Admin password",
    adminLoginButton: "Sign in",
    adminLoginError: "Wrong username or password. Please try again.",
    adminLogoutButton: "Log out from admin",
    manageEvents: "Manage Events",
    manageVenues: "Manage Venues",
    manageBookings: "Manage Bookings",
    scanTicketQr: "Scan Ticket QR",
    scanHint: "Use your device camera to validate and consume ticket QR codes.",
    qrValidUsed: "Ticket is valid and marked as used.",
    invalidTicket: "Invalid ticket.",
    scanFailed: "Scan failed",
    scanConfirmQuestion: "Mark this ticket as scanned?",
    scanConfirmYes: "Yes, mark as scanned",
    scanConfirmNo: "No",
    scanTicketInfo: "Ticket info",
    section: "Section",
    seats: "Seats",
    cameraStartFailed: "Could not start camera",
    cameraPermissionDenied:
      "Camera access was denied. Please allow camera permission for this site in browser settings and reload the page.",
    createEvent: "Create event",
    editEvent: "Edit event",
    edit: "Edit",
    saveEventChanges: "Save changes",
    cancelEditing: "Cancel editing",
    editingEventBadge: "Editing mode",
    eventName: "Event name",
    description: "Description",
    eventImage: "Event image",
    removeImage: "Remove image",
    selectVenue: "Select venue",
    activeEvent: "Active event",
    saving: "Saving...",
    createEventBtn: "Create event",
    existingEvents: "Existing events",
    active: "active",
    inactive: "inactive",
    markActive: "Mark active",
    markInactive: "Mark inactive",
    venueEditor: "Venue editor",
    createNewVenue: "Create new venue",
    new: "New",
    venueName: "Venue name",
    venueAddressLine: "Venue address",
    layoutMeta: "Layout metadata JSON",
    seatsJson: "Seats JSON array",
    generateGrid: "Generate 48-seat grid",
    updateVenue: "Update venue",
    createVenueBtn: "Create venue",
    seatPreview: "Seat map preview",
    filterStatus: "Filter status",
    all: "All",
    pending: "pending",
    paid: "paid",
    cancelled: "cancelled",
    seatsCount: "seats",
    row: "Row",
    seatPlace: "seat",
    markPaid: "Mark paid",
    cancel: "Cancel",
    share: "Share",
    print: "Print",
    linkCopied: "Link copied",
    cancelBooking: "Cancel booking",
    cancelCause: "Select cancellation reason",
    cancelCausePlaceholder: "Describe the reason...",
    cancelCauseOther: "Other",
    venueNameRequired: "Venue name is required.",
    vip: "VIP",
    regular: "Regular",
    balcony: "Balcony",
    seatType: "Seat type",
    price: "Price",
    addTypePrice: "Add type and price",
    remove: "Remove",
    typePriceRequired: "Add at least one price tier (seat type or row).",
    rowPrices: "Row prices",
    typeColors: "Seat type colors",
    bookedSeat: "Booked",
    paidSeat: "Paid",
    availableSeat: "Available",
    selectedSeat: "Selected",
    noSelectedSeats: "No seats selected yet",
    availableSeats: "Available seats",
    takenPlaces: "Taken seats",
    noTakenPlaces: "No taken seats",
    currentTimezone: "Current timezone: {timezone}",
  },
} as const;

export type I18nKey = keyof (typeof dictionary)["ru"];
const TRANSLATABLE_BOOKING_STATUSES = ["pending", "paid", "cancelled"] as const;
type TranslatableBookingStatus = (typeof TRANSLATABLE_BOOKING_STATUSES)[number];

export function t(
  lang: Language,
  key: I18nKey,
  params?: Record<string, string | number>,
): string {
  let text: string = dictionary[lang][key] ?? dictionary.ru[key];
  if (!params) return text;
  for (const [paramKey, value] of Object.entries(params)) {
    text = text.replaceAll(`{${paramKey}}`, String(value));
  }
  return text;
}

export function translateBookingStatus(lang: Language, status: string): string {
  const normalizedStatus = status.trim().toLowerCase();
  if (
    TRANSLATABLE_BOOKING_STATUSES.includes(
      normalizedStatus as TranslatableBookingStatus,
    )
  ) {
    return t(lang, normalizedStatus as TranslatableBookingStatus);
  }

  return status;
}

const CANCEL_CAUSES: Record<Language, string[]> = {
  ru: [
    "Клиент сам отказался",
    "Оплата не поступила",
    "Техническая ошибка",
    "Мероприятие отменено",
    "Мероприятие перенесено",
    "Недостоверные данные клиента",
    "Превышено время ожидания оплаты",
    "Другое",
  ],
  en: [
    "Client cancelled",
    "Payment not received",
    "Technical error",
    "Event cancelled",
    "Event rescheduled",
    "Invalid client details",
    "Payment timeout exceeded",
    "Other",
  ],
};

export function getCancelCauses(lang: Language): string[] {
  return CANCEL_CAUSES[lang];
}
